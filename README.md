...
AgentDesk 提供面向 AI Agent 的 Windows 远程控制能力：

- 让 AI Agent 连接并管理远程 Windows 会话
- 查询 Windows 系统、窗口、进程和文件信息
- 获取屏幕截图，辅助理解当前桌面状态
- 操作剪贴板、键盘、鼠标和窗口
- 在授权范围内执行 PowerShell 和其他受控操作
- 对工具调用进行身份认证、权限控制、人工确认和审计记录
- 通过统一接口适配不同的 AI 客户端和远程连接方式

项目希望补上 AI Agent 从“只能操作本地电脑或 SSH Linux”到“能够安全控制远程 Windows 桌面”之间的能力缺口。

> 当前项目处于早期设计阶段，尚无可运行的控制层、Windows Executor 或 RustDesk 业务消息集成。上面的能力是项目规划目标，尚未完成真实环境验证。
...
当前 AgentDesk 目录包含本说明、协作约定及 `docs/` 文档树。工作区上级目录中的 RustDesk 和 Codex CLI 源码不属于本项目。
